...
def convert_hysteria2(node: dict[str, Any]) -> dict[str, Any]:
    ...
    if params.get("up") or params.get("upmbps"):
        proxy["up"] = params.get("up") or params["upmbps"]
    if params.get("down") or params.get("downmbps"):
        proxy["down"] = params.get("down") or params["downmbps"]
    if params.get("obfs"):
